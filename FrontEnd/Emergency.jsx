...
const Emergency = ({ lang, t }) => {
  ...
  // Filtering logic: checks English name, Urdu name, and the phone number itself
  ...
  return (
    <div className="animate-in slide-in-from-bottom-8 duration-700 max-w-6xl mx-auto py-10 px-4">
      <div className="text-center mb-12">
        <h2 className="text-5xl font-black text-[#2C2621] mb-6 uppercase tracking-tight">
          {t.emergency}
        </h2>
        ...
        <div className="max-w-md mx-auto relative group">
          <input 
            ...
            className={`w-full p-5 rounded-2xl border-2 border-[#2C2621]/10 outline-none focus:border-[#A68A56] transition-all bg-white shadow-sm text-lg ${lang === 'ur' ? 'text-right font-urdu' : 'text-left'}`}
          />
          <div className={`absolute top-1/2 -translate-y-1/2 text-[#A68A56] opacity-30 group-focus-within:opacity-100 transition-opacity ${lang === 'ur' ? 'left-5' : 'right-5'}`}>
            🔍
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredNumbers.map((item, index) => (
            <button
              ...
              className="group relative flex flex-col items-center justify-center p-8 bg-white border-b-4 border-[#2C2621]/10 hover:border-[#A68A56] shadow-sm rounded-2xl transition-all hover:-translate-y-1 active:scale-95 overflow-hidden"
            >
              ...
              <span className="text-sm font-bold uppercase tracking-widest text-[#2C2621]/40 mb-2 group-hover:text-[#A68A56]">
                {item.en}
              </span>
              ...
              <span className="text-4xl font-black text-[#A68A56] tracking-tighter">
                {item.ph}
              </span>
            </button>
          ))}
      ...
      <div className="mt-16 p-8 bg-[#2C2621] rounded-3xl text-center text-white">
