...
const Navbar = ({ lang, setLang, t, setCurrentPage, userName, setUserName, onLogin }) => {
  ...
  const mobileLinkClass = `w-full text-left font-black uppercase py-6 border-b border-[#2C2621]/10 text-[#065016]
      ...
      <nav className="bg-[#FDFBF7]/95 backdrop-blur-2xl">
        <div className="max-w-[1440px] mx-auto px-6 lg:px-10 h-20 lg:h-24 flex justify-between items-center" dir="ltr">
          ...
          <div className="flex items-center gap-4 cursor-pointer group z-50" onClick={() => { setCurrentPage('home'); closeSidebar(); }}>
            ...
            <h1 className="text-xl lg:text-2xl font-black tracking-tighter text-[#065016] hidden sm:block">
              Qanoon Sathi AI
            </h1>
          </div>
          ...
          <div className="flex items-center gap-4 lg:gap-8 flex-1 justify-end">
            ...
            <div className="hidden sm:flex items-center bg-white rounded-full p-1 border-2 border-[#065016]/20 shadow-inner">
              <button 
                onClick={() => setLang('en')} 
                className={`px-4 py-1 text-[11px] font-black rounded-full transition-all duration-500 ${lang === 'en' ? 'bg-blue-600 text-white shadow-lg scale-105' : 'text-gray-400 hover:text-blue-400'}`}
              >
                EN
              </button>
              <button 
                onClick={() => setLang('ur')} 
                className={`px-5 py-1 text-lg font-black rounded-full transition-all duration-500 font-urdu ${lang === 'ur' ? 'bg-red-600 text-white shadow-lg scale-105' : 'text-gray-400 hover:text-red-400'}`}
              >
                اردو
              </button>
            </div>
            ...
            {userName ? (
              <div className="hidden lg:flex items-center gap-4">
                <span className={`font-bold text-[#065016] border-b-2 border-[#A68A56] ${lang === 'ur' ? 'text-xl font-urdu' : 'text-sm'}`}>{userName}</span>
                <button onClick={() => setUserName('')} className="text-[#A68A56] font-black uppercase text-[10px] tracking-widest hover:text-[#065016] transition-colors">{t.logout}</button>
              </div>
            ) : (
              <button 
                onClick={onLogin}
                className="hidden lg:block bg-[#065016] text-[#FDFBF7] hover:bg-[#A68A56] transition-all rounded-full font-black uppercase tracking-widest text-[10px] px-8 py-3 shadow-lg active:scale-95"
              >
                {t.login}
              </button>
            )}
            ...
            >
              <div className={`w-6 h-0.5 bg-[#065016] transition-all duration-300 ${isSidebarOpen ? 'rotate-45 translate-y-2' : ''}`}></div>
              <div className={`w-6 h-0.5 bg-[#065016] transition-all duration-200 ${isSidebarOpen ? 'opacity-0' : ''}`}></div>
              <div className={`w-6 h-0.5 bg-[#065016] transition-all duration-300 ${isSidebarOpen ? '-rotate-45 -translate-y-2' : ''}`}></div>
            </button>
          </div>
        </div>
          ...
          <div className={`absolute right-0 top-0 h-full w-[75%] bg-[#FDFBF7] shadow-2xl transition-transform duration-500 ease-out flex flex-col p-10 pt-32 ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'}`} dir={lang === 'ur' ? 'rtl' : 'ltr'}>
            ...
            <div className="mt-auto space-y-6">
              ...
              {userName ? (
                ...
                <button onClick={() => { onLogin(); closeSidebar(); }} className="w-full py-5 bg-[#065016] text-white rounded-2xl font-black uppercase shadow-xl active:scale-95 hover:bg-[#A68A56] transition-all">
                  {t.login}
                </button>
              )}
            </div>
